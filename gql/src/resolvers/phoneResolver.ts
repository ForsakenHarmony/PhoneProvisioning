...
export class PhoneResolver {
  ...
  private getPhoneApiById(id: string): Promise<PhoneApi> {
    return this.phoneRepository
      .findOneOrFail(id)
      .then(this.getPhoneApi.bind(this));
  }
  ...
  async addPhone(
    ...
    @Arg("phone") phoneInput: PhoneInput
  ): Promise<Phone> {
    const company = await this.companyRepository.findOneOrFail(companyId, {
      relations: ["phones"]
    });
  ...
  async movePhones(
    ...
    return moveEntity(this.phoneRepository, from, to, "company", "phones");
  }
  ...
  async removePhone(@Arg("id", type => ID) id: string): Promise<Phone> {
    ...
    await this.phoneRepository.remove(phone, { transaction: true });
  ...
  async addTopSoftkey(
    @Arg("phoneId", type => ID) phoneId: string,
    @Arg("softkey") topSoftkeyInput: TopSoftkeyInput
  ): Promise<TopSoftkey> {
    const phone = await this.phoneRepository.findOneOrFail(phoneId, {
      relations: ["topSoftkeys"]
    });
  ...
  async addSoftkey(
    @Arg("phoneId", type => ID) phoneId: string,
    @Arg("softkey") softkeyInput: SoftkeyInput
  ): Promise<Softkey> {
    const phone = await this.phoneRepository.findOneOrFail(phoneId, {
      relations: ["softkeys"]
    });
  ...
  async updateSoftkey(
    @Arg("id", type => ID) id: string,
    @Arg("softkey") topSoftkeyInput: SoftkeyInput
  ): Promise<Softkey> {
  ...
  async transferConfigToPhone(@Arg("phoneId", type => ID) phoneId: string) {
    ...
    await api.updateTopSoftkeys(
      await phone.topSoftkeys,
      phone.skipContacts ? [] : phones.filter(p => p.id !== phone.id)
    );
  ...
  async phoneStatus(
    @Root() payload: PhoneNotificationPayload
