...
export function parseForm(doc: string) {
  ...
  const object: ({ tagName: string, content: { [key: string]: string } })[] = (openTags
    .map(tag => tag.match(/<(\w+)\s*(.*?)>/)) as RegExpMatchArray[])
    ...
    .map(({ tagName, content: text }) => {
      ...
      const content = (pairs
        .map(kv => kv.match(/([^\s]+?)="([^"]+)"/)) as RegExpMatchArray[])
        .map(({ 1: key, 2: value }) => ({ [key]: value }))
        .reduce((acc, val) => Object.assign(acc, val), {});
      return { tagName, content };
    });

  const { forms } = object.reduce((acc: { forms: Form[], active: string }, { tagName, content: { id, action, name, value, selected, type } }) => {
